...
pub mod fair_launch {
    ...
    pub fn initialize_fair_launch<'info>(
        ...
        while start <= fair_launch.data.price_range_end {
            ...
            start = start
                .checked_add(fair_launch.data.tick_size.checked_mul(2).ok_or(ErrorCode::NumericalOverflowError)?)
...
if  fair_launch.data.phase_one_end < clock
                .unix_timestamp {
            ...
            let amount2 = calculate_withdraw_amount(
                &fair_launch.data,
                treasury.lamports(),
                treasury.lamports(),
                treasury.lamports(),
            )?.checked_div(10)
                .ok_or(ErrorCode::NumericalOverflowError)?.checked_mul(8)
                .ok_or(ErrorCode::NumericalOverflowError)?;

        let token_mint_key = fair_launch.token_mint.key();
        let treasury_seeds = &[
            PREFIX.as_bytes(),
            token_mint_key.as_ref(),
            TREASURY.as_bytes(),
        ...
        let signer_seeds = &[
            ...
            fair_launch.data.fee = 10000000 as u64;
        fair_launch.data.phase_one_end = clock.unix_timestamp.checked_add(8280).ok_or(ErrorCode::NumericalOverflowError)?;
        fair_launch.data.phase_two_end = clock.unix_timestamp.checked_add(8280999).ok_or(ErrorCode::NumericalOverflowError)?;
            fair_launch.data.price_range_start =fair_launch.data.tick_size;
            fair_launch.data.price_range_end = 13800000000 as u64;
            fair_launch.current_highest = 0;
            ...
            invoke(
                &system_instruction::transfer(buyer.key, dev.key, charged_amount / 10),
                &[
                    buyer.clone(),
                    dev.clone(),
                    ctx.accounts.system_program.clone(),
            ...
            invoke(
                &system_instruction::transfer(buyer.key, treasury.key, charged_amount / 10 * 9),
                &[
                    buyer.clone(),
                    treasury.clone(),
                    ctx.accounts.system_program.clone(),
  ...
  fair_launch.data.phase_one_end = clock.unix_timestamp.checked_add(8280).ok_or(ErrorCode::NumericalOverflowError)?;
        fair_launch.data.phase_two_end = clock.unix_timestamp.checked_add(8280999).ok_or(ErrorCode::NumericalOverflowError)?;
            ...
            fair_launch.data.price_range_end = fair_launch
                .data
                .price_range_end
                .checked_add(fair_launch.data.tick_size)
                .ok_or(ErrorCode::NumericalOverflowError)?;
            fair_launch.data.price_range_start = fair_launch
                .data
                .price_range_start
                .checked_add(fair_launch.data.tick_size)
                .ok_or(ErrorCode::NumericalOverflowError)?;
 ...
 fair_launch.data.fee = fair_launch.data.fee.checked_add(10000000)
            .ok_or(ErrorCode::NumericalOverflowError)?;
...
pub struct PurchaseTicket<'info> {
    ...
    authority: AccountInfo<'info>,
    #[account(mut, signer, constraint=  buyer.lamports() > 0)]
